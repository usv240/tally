...
| 3:10 to 3:35 | Live: the evening digest, then the Month tab. | "At half past six a Strands Graph closes the day, drafts a note home for every child in that family's language from what was actually logged, checks the compliance clock, and puts two questions to her. Two, because the budget is enforced in code. The month's claim is computed from the published rates, and it shows exactly what the unpaid meal cost." |
| 3:35 to 3:55 | Live: the Agent trace. Then the architecture diagram. | "Every step is in the trace: the vision reading, the rule version each verdict was decided under, and the decision whether to interrupt her. The rules are versioned data, published at an endpoint so a sponsor can check them against the USDA tables rather than trusting the agent. The month's money is computed inside Amazon Bedrock AgentCore Code Interpreter, from the same kernel file the local path imports, and the screen says which one answered. Ninety-seven tests, and a vision eval at a hundred percent component recall over thirteen real photographs." |
| 3:55 to 4:10 | The landing page, light then dark, phone then desktop. Title card with the repo and live URL. | "Send a photograph, get a compliant record. Tally. So the paperwork is done before the food is cold." |
