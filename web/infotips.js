...
window.INFOTIPS = {
  where_it_runs: "The claim arithmetic runs inside Amazon Bedrock AgentCore Code Interpreter, using the same kernel.py file the local path imports, so the two cannot drift. If AgentCore does not answer, the identical code runs locally and this line says so rather than pretending. The web tier is AWS App Runner.",
  /* Domain terms ------------------------------------------------------- */
